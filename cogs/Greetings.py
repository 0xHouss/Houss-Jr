...
class Greetings(Cog):
    ...
    async def on_member_join(self, member: Member):
        if not member.bot:
            await self.cursor.execute("""SELECT greetingsChannelId FROM "configs" WHERE guildId = ?""", (member.guild.id,))
            ...
            if greetings_channel_id[0]:
                ...
                joined = Embed(title="Welcome!", description=f"{member.mention} has joined the server!", color=Color.blue())
    ...
    async def on_member_remove(self, member: Member):
        if not member.bot:
            ...
            if greetings_channel_id[0]:
                ...
                left = Embed(title="Goodbye!", description=f"{member.mention} has left the server!", color=0xffa500)
